...
namespace mml {
  ...
  public:
    frame_size_calculator(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<mml::symbol> &symtab, std::shared_ptr<mml::symbol> symbol) :
        basic_ast_visitor(compiler), _symtab(symtab), _symbol(symbol), _localsize(0) {
    }
